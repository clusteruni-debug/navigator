...
function renderCommuteDirectionCard(direction, trip, rhythm, routes, viewDate, expanded) {
  ...
  const departKey = direction === "morning" ? "homeDepart" : "workDepart";
  const arriveKey = direction === "morning" ? "workArrive" : "homeArrive";
  ...
  if (viewDate === getLocalDateStr()) {
    html += '<div class="commute-rhythm-actions"><button type="button" class="commute-rhythm-btn' + (meta.depart ? ' recorded' : '') + '" onclick="handleLifeRhythmClick(\'' + departKey + '\', ' + (meta.depart ? 'true' : 'false') + ', event)">' + escapeHtml(meta.from) + '</button><button type="button" class="commute-rhythm-btn' + (meta.arrive ? ' recorded' : '') + '" onclick="handleLifeRhythmClick(\'' + arriveKey + '\', ' + (meta.arrive ? 'true' : 'false') + ', event)">' + escapeHtml(meta.to) + '</button></div>';
  }
