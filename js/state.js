...
function loadState() {
  try {
    ...
    // 설정 로드
    const parsedLoopStats = safeParseJSON("navigator-loop-stats", null);
    if (parsedLoopStats) appState.loopStats = typeof normalizeLoopStats === "function" ? normalizeLoopStats(parsedLoopStats) : parsedLoopStats;

    const parsedDailyLoop = safeParseJSON("navigator-daily-loop", null);
    if (parsedDailyLoop) appState.dailyLoop = typeof normalizeDailyLoopState === "function" ? normalizeDailyLoopState(parsedDailyLoop) : { ...appState.dailyLoop, ...parsedDailyLoop };

    const parsedPendingEventStatusSync = safeParseJSON("navigator-pending-event-status-sync", null);
    if (Array.isArray(parsedPendingEventStatusSync)) appState.pendingEventStatusSync = parsedPendingEventStatusSync;
...
function _doSaveState(immediate = false) {
  try {
    ...
    if (!appState.user || !isIndexedDBAvailable) {
      ...
      localStorage.setItem("navigator-loop-stats", JSON.stringify(typeof normalizeLoopStats === "function" ? normalizeLoopStats(appState.loopStats) : (appState.loopStats || {})));
      localStorage.setItem("navigator-daily-loop", JSON.stringify(typeof normalizeDailyLoopState === "function" ? normalizeDailyLoopState(appState.dailyLoop) : (appState.dailyLoop || {})));
      localStorage.setItem("navigator-pending-event-status-sync", JSON.stringify(appState.pendingEventStatusSync || []));
    }
