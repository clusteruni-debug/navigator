...
function getCompletionLogKey(dateKey, entry) {
  if (entry && entry.eventId) return dateKey + '|event:' + String(entry.eventId);
...
function isCompletionLogEntryDeleted(dateKey, entry) {
  ...
  if (deleted[getCompletionLogKey(dateKey, entry)]) return true;
...
function addCompletionLogForEvent(event, completedAt) {
  ...
  if (rv) entry.rv = rv;
  if (isCompletionLogEntryDeleted(dateKey, entry)) return false;
...
function removeCompletionLogForEvent(eventId, fallbackTitle) {
  ...
  for (const [dateKey, entries] of Object.entries(appState.completionLog || {})) {
    const idx = (entries || []).findIndex(entry => {
      if (target && String(entry.eventId || '') === target) return true;
      return !entry.eventId && fallbackTitle && entry.t === fallbackTitle && entry.c === '부업';
    });
    if (idx === -1) continue;
...
function backfillSupabaseEventCompletionLog(events) {
  ...
  (events || []).forEach(event => {
    ...
    if (addCompletionLogForEvent(event, new Date().toISOString())) added++;
  });
  if (added > 0) {
    saveCompletionLog();
    saveState();
