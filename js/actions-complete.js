...
function completeTaskForDate(id, dateStr) {
  ...
  // 마일스톤 crossed 판정용 — recompute 전 값 보관 (completeTask 와 대칭)
  const _prevCompletedToday = appState.todayStats.completedToday;
  if (isToday) {
    // 논리일 기준 재계산 (백데이트는 isToday=false 라 제외; 오늘 완료만 반영)
    recomputeTodayStats();
  ...
  if (wasBackdateReset) {
    ...
    if (isToday) checkMilestone(_prevCompletedToday); // 백데이트는 마일스톤 미달성 (의도적)
  }
...
function completeTask(id) {
  ...
  checkMilestone(_prevCompletedToday);
...
function uncompleteTask(id) {
  ...
  if (task.completedAt) {
    ...
    if (appState.completionLog[logDate]) {
      const idx = appState.completionLog[logDate].findIndex(
        // 신규 기록은 id로 정확 매칭, 구 기록은 제목+시각 폴백 (동명+동분 충돌 방지)
        e => (eventId && e.eventId && String(e.eventId) === String(eventId)) || (e.id && e.id === id) || (!e.id && e.t === task.title && e.at === logTime)
      );
      if (idx !== -1) {
        // Soft-Delete: 동기화 부활 방지. tombstone 키는 찾은 기록의 실제 t/at 사용
        // (id로 찾았는데 기록의 title/시각이 task와 달라 키 불일치로 부활하던 문제 방지)
        const _entry = appState.completionLog[logDate][idx];
        const delKey = typeof getCompletionLogKey === "function" ? getCompletionLogKey(logDate, _entry) : logDate + "|" + (_entry.t || task.title) + "|" + (_entry.at || logTime);
  ...
  if (wasCompletedToday) {
    // completedToday는 논리일 기준 재계산 (complete 경로와 대칭 — 수동 -1 은 새벽 경계서 drift)
    recomputeTodayStats();
