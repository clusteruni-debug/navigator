...
function _renderStageTabs(project, activeIdx) {
  ...
  }).join('') + '<button class="work-add-cta work-add-cta--ghost" onclick="promptAddStage(\'' + escapeAttr(project.id) + '\')" aria-label="새 단계 추가">' + _workIcon('plus', 14) + '<span>새 단계</span></button>' + '</div>';
}
