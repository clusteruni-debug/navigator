...
function _stageStats(stage) {
  // 단계 진행률은 "중분류" 단위로 센다 (작업 항목 수가 아니라).
  // 중분류 완료 판정은 _renderSubcategory 의 allDone 과 동일:
  //   task 가 있으면 전부 완료 / 비었으면 _completed 플래그.
  const subcats = stage.subcategories || [];
  const isSubcatDone = (sc) => {
    const tasks = sc.tasks || [];
    return tasks.length > 0
      ? tasks.every(task => task.status === 'completed')
      : !!sc._completed;
  };
  return { total: subcats.length, done: subcats.filter(isSubcatDone).length };
}
