...
function deleteTask(id) {
  ...
  // 휴지통 진입 → trash tombstone 제거 (남아있으면 다음 sync에 휴지통서 증발)
...
function bulkDeleteEvents() {
  ...
  _eventBulkSelectedIds.forEach(id => {
    ...
    // 휴지통 진입 → trash tombstone 제거 (다음 sync 증발 방지)
    if (appState.deletedIds.trash) delete appState.deletedIds.trash[id];
...
function restoreFromTrash(id) {
  ...
  // tasks 병합엔 다시 포함되도록 tasks tombstone 제거
  delete appState.deletedIds.tasks[id];
  // 휴지통 병합엔 다시 안 들어오도록 trash tombstone 설정
  // (지우면 클라우드 trash 배열이 다음 sync에 그대로 부활시킴 — firebase-sync.js:371 deletedTrash 체크)
