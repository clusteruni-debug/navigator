...
function _renderUnifiedEventCard(event) {
  ...
  const eventTaskAction = event.source === "received"
    ? "<button class=\"event-action-btn\" type=\"button\" onclick=\"createTaskFromSupabaseEvent('" + escapeAttr(event.rawId) + "')\" aria-label=\"" + ariaTitle + " 할 일로 연결\">" + svgIcon("list", 14) + "</button>"
    : "";
