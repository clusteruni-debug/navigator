...
// 핑퐁 방지: 자기가 쓴 timestamp Set (단일 slot 시 디바운스 경계에서 outdated event 통과 race 발생 — Set + 5s cleanup으로 회피)
const ownWriteTimestamps = new Set();
function _recordOwnWrite(ts) {
  ownWriteTimestamps.add(ts);
  setTimeout(() => ownWriteTimestamps.delete(ts), 5000);
}
// lastRealtimeSyncToastTime 제거 — 동기화 수신 토스트 자체를 삭제함

// 서버 ack 검증: Firestore SDK의 waitForPendingWrites() 사용
// persistentLocalCache 환경에서 setDoc resolve는 로컬 IndexedDB 쓰기만 보장.
// waitForPendingWrites()는 현재 로컬 큐의 모든 쓰기가 서버 ack 받을 때까지 대기하는 Promise 반환.
// 네트워크 끊기면 영원히 pending이라 Promise.race로 타임아웃 병행 필수.
const SERVER_ACK_TIMEOUT_MS = 15000; // 15초 — 느린 모바일 허용 + 회사 PC 침묵 빠른 탐지
...
async function logout() {
  try {
    ...
    // 진행 중인 waitForPendingWrites Promise는 appState.user=null 체크로 자체 무시됨
    // (별도 정리 코드 불필요)
...
async function _doSyncToFirebase() {
  ...
  try {
    ...
    await window.firebaseSetDoc(userDoc, {
      ...
      loopStats: typeof normalizeLoopStats === "function" ? normalizeLoopStats(appState.loopStats) : (appState.loopStats || {}),
      dailyLoop: typeof normalizeDailyLoopState === "function" ? normalizeDailyLoopState(appState.dailyLoop) : (appState.dailyLoop || {}),
    ...
    // 핑퐁 방지: 자기가 쓴 timestamp Set에 기록 → onSnapshot에서 has()로 판별 (5초 후 자동 cleanup)
    _recordOwnWrite(writeTimestamp);

    // 서버 ack 인라인 검증 — SDK 네이티브 waitForPendingWrites() 사용
    // setDoc resolve는 persistentLocalCache 환경에서 로컬 IndexedDB 쓰기만 보장.
    ...
    // 상태 전환은 Promise.race 결과 비동기로 처리: 'syncing' → 'synced' (ack) 또는 'error' (timeout)
...
async function loadFromFirebase() {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      appState.workProjects = mergeWorkProjects(appState.workProjects, data.workProjects, appState.deletedIds.workProjects);
      ...
      // 셔틀/테마 동기화
      if (data.loopStats) {
        appState.loopStats = mergeLoopStats(appState.loopStats || {}, data.loopStats);
      }
      if (data.dailyLoop) {
        appState.dailyLoop = mergeDailyLoopState(appState.dailyLoop || {}, data.dailyLoop);
      }
...
function startRealtimeSync() {
  ...
  unsubscribeSnapshot = window.firebaseOnSnapshot(userDoc, { includeMetadataChanges: true }, (doc) => {
    ...
    // 서버 ack 검증 자체는 _doSyncToFirebase의 waitForPendingWrites Promise에서 처리
    ...
    if (doc.exists() && !isSyncing && !isLoadingFromCloud) {
      ...
      // Set 기반: 빠른 연속 쓰기 후 outdated snapshot이 오더라도 5초 내면 has()로 잡힘
      if (data.lastUpdated && ownWriteTimestamps.has(data.lastUpdated)) {
        return; // 자기가 쓴 것 — 무시
      }
