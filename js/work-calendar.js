...
function renderWorkCalendarView() {
  ...
  for (var pi = 0; pi < projects.length; pi++) {
    ...
    // 진행률 계산 — 작업 항목(task) 기준. 카드/타임라인과 동일 기준으로 통일
    // (이전엔 단계 완료 기준이라 같은 프로젝트가 화면마다 다른 %로 보였음)
    var calTasks = getAllProjectTasks(proj);
    var totalTasks = calTasks.length;
    var completedTasks = calTasks.filter(function(t) { return t.status === 'completed'; }).length;
