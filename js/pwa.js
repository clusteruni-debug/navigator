...
function checkDeadlinesAndNotify() {
  ...
  // 알림 dedup 기록: {key: ISO타임스탬프} 맵 (24h 경과분 prune). 구 형식(배열)은 마이그레이션
  let notifiedTasks = {};
  try {
    const _raw = JSON.parse(localStorage.getItem(notifiedKey) || '{}');
    if (Array.isArray(_raw)) {
      _raw.forEach(k => { notifiedTasks[k] = now.toISOString(); });
  ...
  appState.tasks.forEach(task => {
    ...
    if (daysLeft > 0.5 && daysLeft <= 1.5 && !notifiedTasks[taskNotifyKey + '-d1']) {
      ...
      if (currentHour >= 8 && currentHour < 12) {  // 오전 8시~12시 사이
        ...
        notifiedTasks[taskNotifyKey + '-d1'] = now.toISOString();
      }
    }

    // D-Day 알림 (마감 당일, 오전에 한 번)
    if (daysLeft > 0 && daysLeft <= 0.5 && !notifiedTasks[taskNotifyKey + '-dday']) {
      const currentHour = now.getHours();
      if (currentHour >= 8 && currentHour < 12) {  // 오전 8시~12시 사이
        showNotification(
          '🔥 오늘이 마감일!',
        ...
        notifiedTasks[taskNotifyKey + '-dday'] = now.toISOString();
      }
    }

    // 3시간 전 알림 (한 번만)
    if (hoursLeft > 0 && hoursLeft <= 3 && !notifiedTasks[taskNotifyKey + '-3h']) {
      showNotification(
        '🚨 마감 3시간 전!',
        `"${task.title}" 마감이 임박했습니다`
      );
      notifiedTasks[taskNotifyKey + '-3h'] = now.toISOString();
    }

    // 1시간 전 알림 (한 번만)
    if (hoursLeft > 0 && hoursLeft <= 1 && !notifiedTasks[taskNotifyKey + '-1h']) {
      showNotification(
        '⚠️ 마감 1시간 전!',
        `"${task.title}" 지금 바로 시작하세요`
      );
      notifiedTasks[taskNotifyKey + '-1h'] = now.toISOString();
    }
  });
  ...
  // 24시간 지난 알림 기록 삭제 (FIFO 100개 컷이 아니라 실제 경과시간 기준 — 중복 알림 방지)
  const _cutoff = now.getTime() - 24 * 60 * 60 * 1000;
  const _pruned = {};
  Object.keys(notifiedTasks).forEach(k => {
    const ts = Date.parse(notifiedTasks[k]);
    if (!isNaN(ts) && ts >= _cutoff) _pruned[k] = notifiedTasks[k];
  });
  safeLocalStorageSet(notifiedKey, JSON.stringify(_pruned));
}
...
function checkBedtimeReminder(now, notifiedTasks) {
  ...
  // 설정된 시간 전 알림 (한 번만)
  if (minutesUntilBed > 0 && minutesUntilBed <= reminderMinutes && !notifiedTasks[todayKey]) {
    ...
    notifiedTasks[todayKey] = now.toISOString();
  }
}
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.ctrlKey || e.metaKey) {
    switch (e.key.toLowerCase()) {
      case 'n': // 새 작업 추가 → 전역 캡처 바
        e.preventDefault();
        if (typeof focusGlobalCapture === 'function') {
          focusGlobalCapture();
        } else {
