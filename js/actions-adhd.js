 ...
 * 마일스톤 체크 및 축하
 */
function checkMilestone(prevCompleted) {
  ...
  // completed 카운터가 recompute로 임계값을 건너뛸 수 있어 strict === 대신
  // "이번에 임계값을 넘어섰는가"로 판정 (인자 없으면 하위호환 === 동작)
  const crossed = (threshold) => (typeof prevCompleted === 'number')
    ? (completed >= threshold && prevCompleted < threshold)
    : (completed === threshold);

  // 일일 목표 달성
  if (crossed(dailyGoal)) {
    showCelebration('🎯');
  ...
  // 특정 개수 달성 (crossed: 임계값을 이번에 넘어섰을 때 1회만)
  if (crossed(3)) {
