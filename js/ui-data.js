...
/**
 * 자산관리 앱으로 수익 내보내기
 * 완료된 Task의 수익을 자산관리 transaction 형식으로 변환한다.
 * P0 cleanup moved this integration out of the removed revenue dashboard file.
 */
function exportToAssetManager() {
  ...
  const transactions = completedTasks.map(task => {
    ...
    return {
      ...
      tags: ["navigator", String(task.category || "").toLowerCase()],
      source: "navigator"
    };
  });
