...
project: navigator
status: SHIPPED
status_reason: All 5 phases shipped 2026-05-28 in single session (P0+P1+P2+P3+P4). P4 = lazy write-path migration (entries-model.js applySubtaskToggle + work-toggles.js wrapper). No eager Firestore migration (firebase-admin unavailable + single-user volume). Zero downtime, zero data loss. Browser smoke pending user — subtask check round-trip + auto-note appears in 기록 box atomic.
milestones:
  ...
  - { id: P4, label: "Firebase data migration + integrity verification (zero data loss)", done: true }
decisions_pending: []
blockers: []
depends_on: []
git_strategy: mono
